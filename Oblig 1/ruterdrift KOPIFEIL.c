...
int main(int argc, char **argv){
  ...
  fileInfo = fopen(argv[1], "r");
  if (fileInfo == NULL) {
    printf("Fant ikke fil argument 1\n");
    exit(EXIT_FAILURE);
  }

  fileCommands = fopen(argv[2], "r");
  if (fileCommands == NULL) {
    printf("Fant ikke fil argument 2\n");
    exit(EXIT_FAILURE);
  }
...
void slett_ruter(int slettId){
  ...
  for (e = 0; e < N; e++) {
    int x;
    for (x = 0; x < alleRutere[e]-> number_of_children; x++) {
